...
for (const name of ['answer', 'acceptance', 'deployment-transition', 'approve', 'refresh']) {
...
test('refresh: the adapter maps a handler response onto the Functions context', async () => {
  const { default: handler } = await import('../api/refresh/index.mjs');
  const context = {};
  await handler(context, { method: 'POST', headers: {}, body: {} });
  assert.equal(context.res.status, 401);
});

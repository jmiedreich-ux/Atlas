...
Alongside the pages, every build writes nine files of its own: `state.json` (above), `tokens.css`,
`order.js`, `deploy.js`, `approve.js`, `refresh.js`, `action-modal.js` and `milestone-toggle.js`
copied from the theme, and `staticwebapp.config.json` (below).
